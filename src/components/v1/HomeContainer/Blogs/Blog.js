    ...
    <div className=' p-4 rounded-[10px] blog '>
      <div className='w-full'>
        <Link href={`/blog/${blog.slug}`}>
          <Image src={blog?.frontmatter?.cover_image} className="blog-cover-image" alt={blog.frontmatter.title} width="352" height="180" />
        </Link>
      </div>
      <div className="my-3">
        <ul className="flex flex-wrap text-xs font-medium -m-1">
          ...
          {tag && (
              <li className="m-1">
              <a className="inline-flex text-center text-gray-100 py-1 px-3 rounded-full bg-blue-500 hover:bg-blue-600 transition duration-150 ease-in-out" href="P#0"> {tag} </a>
            </li>
          )}
      ...
      <Link href={`/blog/${blog.slug}`}><h3 className='text-md xl:text-md font-semibold pb-3'>{index + 1}. {" "} {blog.frontmatter.title}</h3></Link>
